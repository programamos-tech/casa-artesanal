...
export async function POST(request: NextRequest) {
  try {
    ...
    if (uploadError) {
      console.error('upload-credit-payment-receipt:', uploadError)
      return NextResponse.json(
    ...
    const { data: urlData } = supabaseAdmin.storage.from('credit-payments').getPublicUrl(filePath)
    ...
    console.error('upload-credit-payment-receipt:', error)
